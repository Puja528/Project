            ...
            @if($debts->count() > 0)
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-700">
                        <thead>
                            <tr>
                                <th class="px-4 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Tipe
                                </th>
                                <th class="px-4 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Nama
                                    Pihak</th>
                                <th class="px-4 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">
                                    Jumlah (Rp)</th>
                                <th class="px-4 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Jatuh
                                    Tempo</th>
                                <th class="px-4 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Bunga
                                    (%)</th>
                                <th class="px-4 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">
                                    Status</th>
                                <th class="px-4 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">
                                    Deskripsi</th>
                                <th class="px-4 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Aksi
                                </th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-700">
                            @foreach($debts as $debt)
                                        <tr>
                                            ...
                                            <td class="px-4 py-4 whitespace-nowrap">
                                                <div
                                                    class="text-sm font-semibold {{ $debt->type === 'piutang' ? 'text-green-400' : 'text-red-400' }}">
                                                    Rp {{ number_format($debt->amount, 0, ',', '.') }}
                                                </div>
                                            </td>
                                            <td class="px-4 py-4 whitespace-nowrap">
                                                <div
                                                    class="text-sm text-gray-300 {{ $debt->is_overdue ? 'text-red-400 font-semibold' : '' }}">
                                            ...
                                            <td class="px-4 py-4 whitespace-nowrap text-sm font-medium">
                                                <div class="flex space-x-2">
                                                    <a href="{{ route('advance.debts.edit', $debt->id) }}"
                                                        class="text-blue-400 hover:text-blue-300 transition px-2 py-1">
                                                        Edit
                                                    </a>
                                                    <span class="text-gray-500">|</span>
                                                    <!-- FORM DELETE YANG WORKING -->
                                                    <form action="{{ route('advance.debts.destroy', $debt->id) }}" method="POST"
                                                        class="inline">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit"
                                                            onclick="return confirm('Hapus data {{ $debt->person_name }}?')"
                                                            class="text-red-400 hover:text-red-300 transition px-2 py-1">
                                                            Hapus
                                                        </button>
                                                    </form>
                                                </div>
                ...
                <!-- Pagination dengan 10 halaman -->
                @if($debts->hasPages())
                    <div class="mt-6">
                        <nav class="flex flex-col md:flex-row items-center justify-between">
                            <!-- Info jumlah data -->
                            <div class="text-sm text-gray-400 mb-4 md:mb-0">
                                Menampilkan {{ $debts->firstItem() ?? 0 }} - {{ $debts->lastItem() ?? 0 }} dari
                                {{ $debts->total() }} entri
                            </div>
                            ...
                            <ul class="flex flex-wrap items-center justify-center space-x-1">
                                ...
                                @if($debts->currentPage() > 1)
                                    <li>
                                        <a href="{{ $debts->url(1) }}"
                                            class="px-3 py-2 bg-gray-700 hover:bg-gray-600 text-gray-300 rounded-lg transition flex items-center"
                                ...
                                @else
                                    <li>
                                        <span
                                            class="px-3 py-2 bg-gray-800 text-gray-500 rounded-lg cursor-not-allowed flex items-center">
                                            &laquo;&laquo;
                                        </span>
                                    </li>
                                @endif
                                ...
                                <!-- Nomor Halaman (Maks 10) -->
                                @php
                                    $current = $debts->currentPage();
                                    $last = $debts->lastPage();

                                    // Tentukan rentang halaman yang akan ditampilkan (maks 10 halaman)
                                    $start = max(1, $current - 4);
                                    $end = min($last, $start + 9);

                                    // Jika end kurang dari 10 dari start, adjust start
                                ...
                                @for($page = $start; $page <= $end; $page++)
                                    @if($page == $current)
                                        <li>
                                            <span class="px-4 py-2 bg-purple-600 text-white font-semibold rounded-lg">
                                                {{ $page }}
                                            </span>
                                        </li>
                                    @else
                                        <li>
                                            <a href="{{ $debts->url($page) }}"
                                ...
                                @if($debts->currentPage() < $debts->lastPage())
                                    <li>
                                        <a href="{{ $debts->url($debts->lastPage()) }}"
                                            class="px-3 py-2 bg-gray-700 hover:bg-gray-600 text-gray-300 rounded-lg transition flex items-center"
                            ...
                            <div class="text-sm text-gray-400 mt-4 md:mt-0">
                                Halaman {{ $debts->currentPage() }} dari {{ $debts->lastPage() }}
                            </div>
                        </nav>

                        <!-- Tampilkan form untuk pilih jumlah data per halaman -->
                        <div class="mt-4 flex items-center space-x-4">
                            <span class="text-sm text-gray-400">Tampilkan per halaman:</span>
                            <div class="flex space-x-2">
                                @php
                                ...
                                @foreach($perPageOptions as $option)
                                    <a href="{{ request()->fullUrlWithQuery(['per_page' => $option, 'page' => 1]) }}"
                                        class="px-3 py-1 rounded text-sm {{ $currentPerPage == $option ? 'bg-purple-600 text-white' : 'bg-gray-700 text-gray-300 hover:bg-gray-600' }}">
                                        {{ $option }}
                                    </a>
                                @endforeach
                            </div>
